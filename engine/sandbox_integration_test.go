...
func TestSandboxApplyAcceptsRecoveryPlanContainedByApprovedMandate(t *testing.T) {
	...
	repository := newSandboxRepository(t)
	target := SandboxTarget{Host: "github.com", OwnerID: "owner", RepositoryID: "repo", ProjectID: "project", RepositoryName: "owner/sandbox"}
	authority := testSandboxAuthority()
	manifest := SandboxManifest{
		SchemaVersion: 1, OperationID: "recovery", SourceRevision: "source-2", ConfigurationRevision: "config",
		ApprovedBy: "owner", ApprovedPlan: "issue-73-mandate", RecoveryOwner: "sandbox-owner", MarkerPrefix: "starter-kit-contract:run-1:", Target: target, Authority: authority,
	...
	adapter := NewInMemorySandboxAdapter(
		SandboxCapability{SchemaVersion: 1, Available: true, Fresh: true, Actor: "app", EvidenceMode: "memory", Compatibility: authority.Compatibility, Target: target, Permissions: authority.Permissions, CredentialIdentities: authority.CredentialIdentities, ConfigurationRevision: "config", ObservedAt: now, ExpiresAt: now.Add(time.Hour)},
	...
	mandate := SandboxExecutionMandate{
		...
		Target: target, Actors: []string{"app"}, MarkerPrefix: "starter-kit-contract:run-1:", ResourceKinds: []string{SandboxResourceFixtureIssue}, EffectKinds: []string{"remove-resource"}, MaxEffects: 3, DataClass: "public-synthetic", CostCeiling: "zero-dollar", Destructive: "marker-scoped-cleanup", Retention: "30-days", RecoveryOwner: "sandbox-owner", Authority: authority,
	}
	mandate = BindSandboxExecutionMandate(mandate, manifest.Resources...)
...
func TestSandboxApplyRejectsPlanOutsideApprovedMandate(t *testing.T) {
	...
	target := SandboxTarget{Host: "github.com", OwnerID: "owner", RepositoryID: "repo", ProjectID: "project", RepositoryName: "owner/sandbox"}
	authority := testSandboxAuthority()
	approved := SandboxResourceSpec{Key: "fixture:branch:cleanup", Kind: SandboxResourceFixtureBranch, Name: "contract/run/cleanup", Marker: "starter-kit-contract:run-1:branch:cleanup", DesiredState: SandboxResourceAbsent, Attributes: map[string]string{}}
	forged := approved
	forged.Name = "main"
	plan := SandboxPlan{SchemaVersion: 1, Repository: newSandboxRepository(t), OperationID: "operation", SourceRevision: "source", ConfigurationRevision: "config", Target: target, RecoveryOwner: "sandbox-owner", Authority: authority, Effects: []SandboxEffect{{Kind: "remove-resource", Resource: forged}}}
	plan.ID = digestJSON(plan)
	mandate := SandboxExecutionMandate{SchemaVersion: 1, ApprovedBy: "owner", ApprovalID: "approval", ApprovedAt: now.Add(-time.Hour), ExpiresAt: now.Add(time.Hour), Target: target, Actors: []string{"app"}, MarkerPrefix: "starter-kit-contract:run-1", ResourceKinds: []string{SandboxResourceFixtureBranch}, EffectKinds: []string{"remove-resource"}, MaxEffects: 1, DataClass: "public-synthetic", CostCeiling: "zero-dollar", Destructive: "marker-scoped-cleanup", Retention: "30-days", RecoveryOwner: "sandbox-owner", Authority: authority}
	mandate = BindSandboxExecutionMandate(mandate, approved)
...
func TestSandboxInspectionRejectsCredentialPermissionExpansion(t *testing.T) {
	now := time.Date(2026, 7, 17, 21, 0, 0, 0, time.UTC)
	target := SandboxTarget{Host: "github.com", OwnerID: "owner", RepositoryID: "repo", ProjectID: "project", RepositoryName: "owner/sandbox"}
	authority := testSandboxAuthority()
	manifest := SandboxManifest{SchemaVersion: 1, OperationID: "operation", SourceRevision: "source", ConfigurationRevision: "config", ApprovedBy: "owner", ApprovedPlan: "approved", RecoveryOwner: "sandbox-owner", MarkerPrefix: "starter-kit-contract:run-1", Target: target, Authority: authority}
	expanded := cloneSandboxAuthority(authority)
	expanded.Permissions = append(expanded.Permissions, "app:workflows:write")
	adapter := NewInMemorySandboxAdapter(
		SandboxCapability{SchemaVersion: 1, Available: true, Fresh: true, Actor: "app", EvidenceMode: expanded.EvidenceMode, Compatibility: expanded.Compatibility, Target: target, Permissions: expanded.Permissions, CredentialIdentities: expanded.CredentialIdentities, ConfigurationRevision: "config", ObservedAt: now, ExpiresAt: now.Add(time.Hour)},
		SandboxObservation{SchemaVersion: 1, Target: target, ConfigurationRevision: "config"},
	)
	lifecycle := New(WithClock(sandboxFixedClock{now}), WithSandboxAdapter(adapter))
	...
	if inspection.Disposition != "non-pass" || !strings.Contains(strings.Join(inspection.Problems, ";"), "permissions") {
		t.Fatalf("inspection = %#v", inspection)
	}
}

func testSandboxAuthority() SandboxAuthorityProfile {
	return SandboxAuthorityProfile{CredentialIdentities: []string{"app|test|actor|account|1|2"}, Permissions: []string{"app:issues:write"}, EvidenceMode: "memory", Compatibility: "test-native", DataClass: "public-synthetic", CostCeiling: "zero-dollar", Destructive: "marker-scoped-cleanup", Retention: "30-days"}
}
