...
|---|---|---|---|---|
| Exact replay after `applied` | Managed contract and prior event match the immutable plan | Stable `applied`; no changed files | Return prior semantic outcome without mutation | #30 proves native equivalence of this observable behavior |
| Exact replay after `no_change` | Managed contract and no-change event match the plan | Stable `no_change`; no changed files | Return prior semantic outcome without mutation | Same #30 runtime-publication dependency |
...
- Secret scanning remains `not-configured`; recovery passing does not make aggregate seed
  verification pass.
