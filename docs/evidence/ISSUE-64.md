...
| `Plan` | Lifecycle-engine immutable semantic delta with source, observation, repository, Project, field, and option preconditions |
| `EffectReceipt` | Per-effect applied, ambiguous, denied, rate-limited, or recovery result retained across partial failure without claiming distributed atomicity |
...
- lost create response: the result stays `ambiguous` until stable-marker lookup discovers
  exactly one object, preventing blind duplicate creation;
...
documentation validator also passed. The GitHub Actions matrix remains the native Linux,
macOS, and Windows evidence route for the PR.
...
production Work Manager and in-memory adapter absorb the validated boundary. The decision
